...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--spec-max-concurrency"}, "N",
        string_format("skip drafting when more than N slots generate at once, 0 = no limit (default: %d)", params.speculative.max_concurrency),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("invalid value");
            }
            params.speculative.max_concurrency = value;
        }
    ).set_spec().set_examples({LLAMA_EXAMPLE_SERVER}).set_env("LLAMA_ARG_SPEC_MAX_CONCURRENCY"));
    add_opt(common_arg(
        {"--spec-draft-n-max"}, "N",
