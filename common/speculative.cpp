...
common_speculative_init_result::common_speculative_init_result(
    ...
    if (has_draft) {
        ...
        // load the draft path computed above -- passing params.model.path here silently
        // loads the target a second time as the "draft", so every drafted block costs a
        // full-size forward pass and the block drafter runs against the wrong architecture
