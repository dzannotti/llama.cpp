...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    if (hparams.ple_n_heads > 0) {
        ...
        // hoisted out of the `if`: the direct-read path below needs the weight's file
        // index and offset. #28136 was written against an older master where this was a
        // required outer-scope reference (require_weight); master has since made the
        // tensor optional, so it is a possibly-null pointer here and the direct-read
        // path has to be guarded on it.
